...
export default function CustomerNotifications() {
  ...
  const markRead = async (id) => {
    try {
      ...
      setNotes(prev => prev.map(n => (n.id || n.id) === id ? { ...n, isRead: true } : n))
    } catch {}
  }
  ...
  const deleteNote = async (id) => {
    try {
      ...
      setNotes(prev => prev.filter(n => (n.id || n.id) !== id))
      ...
      {notes.length === 0 ? (
        ...
        <div className="space-y-2">
          {notes.map(n => {
            ...
            return (
              <div key={n.id || n.id} onClick={() => markRead(n.id || n.id)}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button onClick={e => { e.stopPropagation(); deleteNote(n.id || n.id) }}
