...
const runTests = async () => {
    try {
        ...
        // --- PHASE 6: ML WAIT TIME PREDICTION ---
        logInfo("--- Phase 6: ML Wait Time Prediction ---");
        const ml1 = await api.post('/queue/predict-wait-time', {
        ...
        logSuccess(`ML Prediction received: ${ml1.data.predictedWaitTime} minutes`);
        if (ml1.data.isFallback) {
            logInfo("Note: The prediction was generated using the Fallback logic (ML service offline).");
        } else {
            logInfo("Note: The prediction was generated by the Python ML service.");
        }
